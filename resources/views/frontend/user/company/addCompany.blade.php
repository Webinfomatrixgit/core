...
@section('user-article-content')
    <form action="{{ route('user.company.add') }}" method="post" enctype="multipart/form-data">
        @csrf
        <div class="card-body">
            <h2 class="mb-4"> {{ __('Add Company') }}</h2>
            ...
            <div class="mb-3">
                ...
                <input type="number" name="phone" id="phone" class="form-control" placeholder="{{ __('Phone') }}" required>
            </div>
            <div class="mb-3">
                <label class="form-label" for="website">{{ __('Website') }}</label>
                <input type="text" name="website" id="website" class="form-control" placeholder="{{ __('Website') }}" required>
            </div>
